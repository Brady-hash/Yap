    ...
    Mutation: {
        ...
        joinThread: async (parent, { userId, threadId }, context) => {
            try {
                ...
                return await MessageThread.findByIdAndUpdate(threadId, { $addToSet: { participants: userId } }, { new: true }).populate('messages').populate('admin').populate('participants').populate({ path: 'messages', populate: { path: 'sender', select: 'username'}});
            } catch(err) {
        ...
        updateQuestion: async (parent, { userId, questionId, text, option1, option2 }) => {
            try {
                ...
                return await Question.findByIdAndUpdate(questionId, { text, option1, option2 }, { new: true }).populate('creator').populate('messageThread');
            } catch(err) {
        ...
        deleteQuestion: async (parent, { userId, questionId }, context) => {
            try {
                ...
                throw new Error('Error deleting questiong', err)
            }
        },
        answerQuestion: async (parent, { userId, questionId, answer }, context) => {
            try {
                ...
                throw new Error('Error answering questiong', err);
            }
        }
    }
}
